 ...
 * <pre>
 *   someModule.controller('Controller', ['ping', function(ping) {
 *     ping();
 *   }]);
 * </pre>
 */
 ...
 * You would then inject and use this service like this:
 * <pre>
 *   someModule.controller 'Controller', ['ping', (ping)->
 *     ping.send()
 *   ]
 * </pre>
 */
 ...
 * @example
 <example module="mySceApp">
  <file name="index.html">
    <div ng-controller="myAppController as myController">
      <i ng-bind-html="myController.explicitlyTrustedHtml" id="explicitlyTrustedHtml"></i><br><br>
      <b>User comments</b><br>
      By default, HTML that isn't explicitly trusted (e.g. Alice's comment) is sanitized when $sanitize is available.  If $sanitize isn't available, this results in an error instead of an exploit.
      <div class="well">
        <div ng-repeat="userComment in myController.userComments">
    ...
    var deferreds = {};
      ...
      * @example
      <doc:example module="time">
        <doc:source>
          <script>
            function Controller2($scope,$timeout) {
...
function FormController(element, attrs) {
  var form = this,
      parentForm = element.parent().controller('form') || nullFormController,
      ...
      compile: function() {
        return {
          pre: function(scope, formElement, attr, controller) {
            ...
            var parentFormController = formElement.parent().controller('form'),
            ...
            if (parentFormController) {
              formElement.on('$destroy', function() {
                parentFormController.$removeControl(controller);
                if (alias) {
                  setter(scope, alias, undefined, alias);
                }
                extend(controller, nullFormController); //stop propagating child destruction handlers upwards
              });
            }
          }
        };
...
var NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse',
  ...
  var parentForm = $element.inheritedData('$formController') || nullFormController,
      invalidCount = 0, // used to easily determine if we are valid
  ...
  return {
    ...
    link: function(scope, element, attr, ctrls) {
      ...
      var modelController = ctrls[0],
          formController = ctrls[1] || nullFormController;
...
var selectDirective = ['$compile', '$parse', function($compile,   $parse) {
  ...
  var NG_OPTIONS_REGEXP = /^\s*(.*?)(?:\s+as\s+(.*?))?(?:\s+group\s+by\s+(.*))?\s+for\s+(?:([\$\w][\$\w]*)|(?:\(\s*([\$\w][\$\w]*)\s*,\s*([\$\w][\$\w]*)\s*\)))\s+in\s+(.*?)(?:\s+track\s+by\s+(.*?))?$/,
      nullModelController = {$setViewValue: noop};
  ...
  return {
    ...
    controller: ['$element', '$scope', '$attrs', function($element, $scope, $attrs) {
      var self = this,
          optionsMap = {},
          ngModelController = nullModelController,
          nullOption,
          unknownOption;
      ...
      self.init = function(ngModelController_, nullOption_, unknownOption_) {
        ngModelController = ngModelController_;
      ...
      self.addOption = function(value) {
        ...
        if (ngModelController.$viewValue == value) {
          $element.val(value);
    ...
    link: function(scope, element, attr, ctrls) {
      ...
      selectController.init(ngModelController, nullOption, unknownOption);

      // required validator
      if (multiple && (attr.required || attr.ngRequired)) {
        var requiredValidator = function(value) {
          ngModelController.$setValidity('required', !attr.required || (value && value.length));
          return value;
        };

        ngModelController.$parsers.push(requiredValidator);
        ngModelController.$formatters.unshift(requiredValidator);

        attr.$observe('required', function() {
          requiredValidator(ngModelController.$viewValue);
        });
      }

      if (optionsExp) Options(scope, element, ngModelController);
      else if (multiple) Multiple(scope, element, ngModelController);
      else Single(scope, element, ngModelController, selectController);
      ...
      function Single(scope, selectElement, ngModelController, selectController) {
        ngModelController.$render = function() {
          var viewValue = ngModelController.$viewValue;

          if (selectController.hasOption(viewValue)) {
            if (unknownOption.parent()) unknownOption.remove();
            ...
            if (isUndefined(viewValue) && emptyOption) {
              ...
              selectController.renderUnknownOption(viewValue);
            }
          }
        };
        ...
        selectElement.on('change', function() {
          scope.$apply(function() {
            if (unknownOption.parent()) unknownOption.remove();
            ngModelController.$setViewValue(selectElement.val());
          });
        });
      }
...
var optionDirective = ['$interpolate', function($interpolate) {
  ...
  return {
    ...
    compile: function(element, attr) {
      ...
      return function (scope, element, attr) {
        var selectControllerName = '$selectController',
            parent = element.parent(),
            selectController = parent.data(selectControllerName) ||
              parent.parent().data(selectControllerName); // in case we are in optgroup

        if (selectController && selectController.databound) {
          // For some reason Opera defaults to true and if not overridden this messes up the repeater.
          // We don't want the view to drive the initialization of the model anyway.
          element.prop('selected', false);
        } else {
          selectController = nullSelectController;
        }

        if (interpolateFn) {
          scope.$watch(interpolateFn, function interpolateWatchAction(newVal, oldVal) {
            attr.$set('value', newVal);
            if (newVal !== oldVal) selectController.removeOption(oldVal);
            selectController.addOption(newVal);
          });
        } else {
          selectController.addOption(attr.value);
        }

        element.on('$destroy', function() {
          selectController.removeOption(attr.value);
        });
      };
    }
  };
